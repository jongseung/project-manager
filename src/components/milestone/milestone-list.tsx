...
export function MilestoneList({ projectId, milestones }: MilestoneListProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      {milestones.length === 0 ? (
        <EmptyState icon={<Diamond className="h-8 w-8" />} title="No milestones" description="Mark key dates in your project." className="py-6" />
      ) : (
        milestones.map((m) => (
          <div key={m.id} className="flex items-center gap-3 rounded-lg border p-3">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {m.status === "pending" && <DropdownMenuItem onClick={() => markReached(m.id)}>Mark as reached</DropdownMenuItem>}
                <DropdownMenuItem className="text-destructive" onSelect={(e) => { e.preventDefault(); setConfirmId(m.id); }}><Trash2 className="h-4 w-4 mr-2" /> Delete</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        ))
